...
export default function UpdateTaskModal({ isOpen, setOpen, data }) {
    const dispatch = useAppDispatch();
    const [labelOpen, setLabelOpen] = useState(false);
    const userInfo = useAppSelector((state) => state.user.info);
    const loading = useAppSelector((state) => state.task.loading);
    const [status, setStatus] = useState(data.status);
    const [assignee, setAssignee] = useState(data.assignees);
    const [label, setLabel] = useState(data.label);
    const [labelValue, setLabelValue] = useState("");
    const [desc, setDesc] = useState(data.description);
    const [dueDate, setDueDate] = useState(data.dueDate);
    const [title, setTitle] = useState(
        data.title ? data.title : "Enter your title ..."
    );
    ...
    const handleUpdateTask = async () => {
        try {
            ...
            !loading && toast.success("📝 Update Successfully");
            setTimeout(() => {
                setOpen(false);
                dispatch(fetchTasks());
            }, 2000);
                ...
                <div className="body">
                    ...
                    <div className="pl-7 pb-7 pr-48">
                        ...
                        <Datepicker
                            ...
                            defaultDate={data.dueDate && new Date(data.dueDate)}
                            className="pt-2"
                        />
                    </div>
                    ...
                    {/* Activity Section */}
                    <Activity userInfo={userInfo} id={data.id} />
