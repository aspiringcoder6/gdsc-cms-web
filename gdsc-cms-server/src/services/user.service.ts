...
export async function getProfile(req: Request, res: Response) {
    ...
    const missingProperties = neededProperties.filter(key => !profileData.hasOwnProperty(key))
...
export const updateProfileValidators = [
    ...
    , body("email").trim()
        ...
        .isLength({ max: 100 })
        .optional()
    , body(["department", "sector", "role", "location"]).trim()
        .isString()
        .isLength({ max: 100 })
        .optional()
    , body("contact").trim()
        ...
        .isLength({ max: 2000 })
        .optional()
    , body(["linkGithub", "linkFacebook", "linkTiktok"]).trim()
        .isString()
        .isLength({ max: 100 })
        .optional()
]
